...
df = pd.read_csv("D:\Programming Projects\Repositories\AeroVision\LSTM 2/normalized_data.csv")
...
features_df = df.drop(columns=["class"])  # shape = (2700, 16)
...
num_features = features_df.shape[1]  # should be 16
X = features_df.to_numpy().reshape(num_sequences, sequence_length, num_features)
print("X.shape:", X.shape)  # (90, 30, 16)
